  ...
  return (
    <div className="pt-[168px] bg-[#F6F9FB] ">
      <Container className="-mb-[190px]">
        <div
          className="grid grid-cols-4 px-12 pb-12 gap-12  bg-gradient-to-b from-[#F6F9FB] from-25% via-[white] via-25% to-white to-100%">
          {
            getAgent.isLoading ? <Skeleton className="h-[415px] w-full"/> :
              <img
          ...
          <div className="col-span-3 flex flex-col justify-end py-6 h-full">
            <div className="mb-24">
              <h3 className="text-gray-500 font-bold text-[40px] mb-4">
                {
                  getAgent.isLoading ? <Skeleton className="h-16" count={0.6}/> : getAgent.data?.name
                }
              </h3>
              <h5 className="text-[#8C959F] text-2xl">
                {getAgent.isLoading ? <Skeleton className="h-9" count={0.4}/> : getAgent.data?.field}
              </h5>
            </div>
            <div className="flex">
              {
                getAgent.isLoading ?
                  <Skeleton count={0.8} className="h-9 w-full" containerClassName="w-full"/> : <>
        ...
        <Container>
          <div className="p-20 border border-[#E3E3E3]">
            <div className="flex gap-6 mb-12">
              {
                getAgent.isLoading ?
                  <Skeleton count={0.3} className="h-full w-full" containerClassName="h-16 w-full"/> :
              ...
              {tab === "About me" && (
                <>
                  <h5 className="text-2xl font-bold text-gray-500 mb-5">
                    {
                      getAgent.isLoading ?
                        <Skeleton count={0.2} className="h-full w-full"
                                  containerClassName="h-8 w-full flex"/> : "About me"
                    }
                  </h5>
                  <p className="text-[#8C959F] text-xl leading-8">
                    {
                      getAgent.isLoading ? <Skeleton count={7.3}/> : getAgent.data?.about
                    }
