    ...
    <Wrapper>
      <div className="grid lg:grid-cols-2">
        <div tw="w-full">
          ...
          <div tw="grid grid-cols-2 mt-20 gap-8">
            {HomelunFeatures.map((feature: TFeatures, index: number) => (
              ...
              >
                <div className="mb-1 min-h-0 md:h-24 px-7 flex flex-col justify-center relative before:absolute before:left-0 before:w-1 before:h-full md:before:h-8 before:bg-red-100 before:top-1/2 before:-translate-y-1/2 group-hover:before:h-full before:transition-all before:ease-in-out group-hover:before:bg-red-500">
        ...
        <div className="w-full relative h-fit">
          <div className="before:-z-10 before:absolute before:right-0 before:top-0 before:h-[82.5%] md:before:h-[81.2%] before:mt-[calc(100%_-_81%)] before:w-full  before:bg-gradient-to-r before:from-white before:from-10% before:via-[#F6F9FB] before:via-10% before:to-[#F6F9FB] before:to-100% before:rounded-b-full min-h-full">
            <div tw="absolute top-0 left-0 w-6/12 sm:w-5/12 h-max bg-white p-4 shadow-sm">
            ...
            <img
              className="rounded-b-full mt-24 md:mt-12 md:min-h-fit w-full"
              src={TopAgent}
            />
          </div>
        </div>
...
const FeaturesWrapper = styled.div<{ index: number; hovered: number }>`
  ${({ hovered, index }) =>
    hovered === index
      ? tw`[& div]:(before:(h-full bg-red-500)) md:[& div>p]:(h-full visible opacity-100 mt-4)`
      : tw``}
`;
const FeatureDesc = tw.p`text-gray-500 h-full md:h-0 text-base leading-7 md:invisible md:opacity-0  transition-all ease-in-out`;
